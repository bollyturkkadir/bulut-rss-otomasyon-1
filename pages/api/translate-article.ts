...
export default async function handler(
  ...
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: "Makale ID'si eksik." });
    }
    ...
    if (!rawArticleDoc.exists) {
      return res
        .status(404)
        .json({ success: false, message: "Makale bulunamadı." });
    }

    const rawArticle = rawArticleDoc.data();

    // Başlığı ve içeriği çevir
    const translatedTitleResult = await translator.translateText(
      rawArticle.title,
      null,
      "tr",
    );
    const translatedContentResult = await translator.translateText(
      rawArticle.content,
      null,
      "tr",
    );
    ...
    res
      .status(200)
      .json({ success: true, message: "Makale başarıyla çevrildi." });
  } catch (error) {
